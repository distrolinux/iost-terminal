...
| Buy/swap interface | Prepared, fail-closed | Phase 4 panel and server-side `evaluateTradingAccess()` require deployed status, token/pair addresses, BSC chain 56 and approved PancakeSwap HTTPS URL | Audited bridge/wrapper, real liquidity pair, refreshed counsel, Phase 4 approval |
...
| Contract testing | Complete | 68 Hardhat tests; full Solidity compile | Must remain green after audit fixes |
...
Trading opens only when every Phase 4 check passes: explicit trading request, Phase 4 enabled, deployed status, token address, BSC chain 56, pair address, exact PancakeSwap venue and an allowlisted HTTPS PancakeSwap URL. Failed checks remove the swap URL from the public API.

...
- **Fail-closed agent execution:** REST paper opens made with a platform or per-user agent credential require a positive `entry` + `size`, an owned agent wallet, and an active wallet-bound Pact. Limit and Pact capacity are reserved before execution and committed only after success. Human-session paper execution is unchanged. This authorization rail is not feature-flagged.

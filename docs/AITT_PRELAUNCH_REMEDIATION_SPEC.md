...
Approved architecture, implementation in progress. AITT remains undeployed; conversion and Phase 4 remain closed.

## Objective

Make AITT's locked 1B supply, single 200M burn cap/800M floor, allocation custody, points conversion, and launch gates enforceable end-to-end before any deployment or external liquidity.

## Approved decisions

1. **Burn authority:** one `AITTFeeRouter` is the sole external protocol-burn path. `AITT` keeps internal AMM burns and a one-time-locked router address. Every burn reduces `totalSupply()` via `_burn`; dead-address burns are forbidden by policy and tooling.
2. **Supply floor:** all burn paths share `totalSupply() - 800M` headroom. Excess burn share redirects 70/30 to stakers/treasury.
...
- No dead-address burn path and no mint after construction.
...
- `executeDaoBurn(amount)`: owner/DAO queues or submits owned AITT through the same protocol-burn path; no dead-address transfers.
...
- global burn path and allocation totals pass verification;
...
- Every burn source reduces `totalSupply()` under one 800M floor.

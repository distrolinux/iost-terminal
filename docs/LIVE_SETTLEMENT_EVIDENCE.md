...
## Conflicting evidence review hold

A structurally valid replacement of a recorded fill (including fee/net-amount or binding changes), or reuse of a recorded ledger ID for another fill, now creates a private, synced, exclusive `review-required.json` marker. It retains a bounded reason and hashes of the conflicting evidence; original records are never overwritten. Any marker presence—including a partial/corrupt marker—keeps subsequent appends and otherwise valid replays held across restarts. Later conflicts never overwrite the first marker. There is no automatic expiry, clear, repair or owner-release API.

Malformed or unavailable observations still fail closed but are not classified as confirmed corrections. New reversal records that do not reuse known IDs are not automatically recognized; discovery and reviewed compensating accounting remain unfinished. The marker is local evidence-review state, not an Incident Center release workflow or a transactional trading lock. Concurrent evidence writers may finish an already-started evidence append; they never gain settlement/execution authority. No production trading gate is relaxed or changed by this internal feature.

## Read-only review summary

`inspectLiveSettlementReview(root, ownerId)` reads only local private evidence and creates no files. It returns counts, local-chain integrity status and a bounded review reason, never order/fill/ledger IDs, amounts, account bindings or hashes. It distinguishes absent evidence, recorded evidence, a required review and unavailable/corrupt evidence. A corrupt marker still requires review. Permission, size, no-follow, schema, hash-chain and duplicate-ID checks fail closed; observed directory changes during inspection return unavailable.

This internal summary is not yet an owner-facing page or MCP tool. A future route must derive the owner from its authenticated session. Local chain verification is not external anchoring, final settlement or proof against privileged deletion/rollback, and the read is not an atomic snapshot against concurrent writers. No clear/acknowledge/repair/release endpoint exists. All settlement and execution authority remains false.

## Signed-in owner panel

The existing owner-session-only `/api/exchange-connections` response now includes the sanitized summary from `IOST_DATA_DIR/live-settlement-history`, deriving the owner exclusively from the session. Agent credentials (including alongside a browser session) remain rejected. Responses are private/no-store. The Live Trading workspace displays the summary with an explicit read-only label and local-evidence limitations; its existing Refresh evidence button rereads the local status only. No separate endpoint, evidence collection, exchange request or clear-hold control is added.

This supersedes the not-yet-owner-facing note above. The panel may honestly show no local evidence because automatic collection remains unwired. HTTP fixtures cover signed-out denial, agent denial, cross-owner query attempts, unchanged evidence and private caching. Browser visual review remains separate from these automated checks. Deployment still requires review of draft #104 and its remaining live-safety blockers.

...
try {
  ...
  assert.equal(own.runs.every((run) => run.challenge?.status === 'verified'), true);
  ...
  const challenges = history.listAgentChallenges('user-a-private-id', 50, now);
  assert.equal(challenges.challenges.length, 3);
  assert.equal(challenges.guarantees.privateByDefault, true);
  assert.equal(challenges.guarantees.automaticPromotion, false);
  assert.equal(challenges.challenges.every((run) => run.challenge?.summary?.scenarioCount === 5), true);

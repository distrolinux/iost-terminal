...
try {
  ...
  const a = holds.claim('owner-a', order, "a".repeat(64)); assert.equal(a.ok, true);
  assert.equal(holds.claim('owner-a', order, "a".repeat(64)).ok, false);
  const file = join(dir, readdirSync(dir)[0]);
  assert.equal(JSON.parse(readFileSync(file)).clientOrderId, a.clientOrderId);
  assert.equal(statSync(file).mode & 0o777, 0o600);
  const script = `import { createLiveSubmissionHold } from ${JSON.stringify(new URL('../lib/live-submission-hold.js', import.meta.url).href)}; const r = createLiveSubmissionHold(${JSON.stringify(dir)}).claim('owner-a', ${JSON.stringify(order)}, "a".repeat(64)); process.exit(r.ok ? 1 : 0);`;
  assert.equal(spawnSync(process.execPath, ['--input-type=module', '-e', script]).status, 0, 'new process blocks persisted owner');
  writeFileSync(file, '{broken');
  assert.equal(holds.claim('owner-a', order, "a".repeat(64)).ok, false, 'corrupt hold cannot be cleared by retry');
  assert.equal(holds.claim('owner-b', order, "a".repeat(64)).ok, true, 'separate owner');
  ...
  const invalid = createLiveSubmissionHold(file).claim('owner-c', order, "a".repeat(64)); assert.equal(invalid.ok, false);

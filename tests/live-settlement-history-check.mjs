...
try {
  assert.equal(inspectLiveSettlementReview(root, 'owner').status, 'not-observed');
  assert.deepEqual(readdirSync(scratch), [], 'inspection creates no directories');
  ...
  const cleanReview = inspectLiveSettlementReview(root, 'owner');
  assert.equal(cleanReview.status, 'evidence-recorded');
  assert.equal(cleanReview.historyIntegrity, 'verified-local-chain');
  assert.equal(cleanReview.fillCount, 1); assert.equal(cleanReview.ledgerCount, 2);
  assert.equal(cleanReview.releaseAllowed, false); assert.equal(cleanReview.settlementVerified, false);
  for (const secret of [binding.venueOrderId, binding.credentialBinding, link.fillId, link.ledgerIds[0]]) assert.ok(!JSON.stringify(cleanReview).includes(secret));
  assert.equal(readFileSync(file, 'utf8'), bytes);
  assert.equal(inspectLiveSettlementReview(root, 'absent-owner').status, 'not-observed');
  ...
  const review = inspectLiveSettlementReview(correctionRoot, 'owner');
  assert.equal(review.status, 'review-required'); assert.equal(review.reasonCode, 'recorded-fill-changed');
  assert.equal(review.historyIntegrity, 'verified-local-chain'); assert.equal(review.ownerReviewRequired, true);
  ...
  assert.equal(inspectLiveSettlementReview(correctionRoot, 'owner').reasonCode, 'settlement-review-marker-unavailable');
  assert.equal(correction.append('owner', binding, evidence).status, 'held');
  ...
  assert.equal(inspectLiveSettlementReview(root, 'owner').status, 'unavailable');

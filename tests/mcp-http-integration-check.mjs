...
try {
  ...
  const securitySentinelTool = privateTools.body.result.tools.find((tool) => tool.name === 'agent_security_sentinel_status');
  const publicExecutionReadinessTool = privateTools.body.result.tools.find((tool) => tool.name === 'public_execution_launch_readiness');
  ...
  assert.equal(securitySentinelTool.annotations.readOnlyHint, true);
  assert.equal(securitySentinelTool.annotations.destructiveHint, false);
  assert.equal(securitySentinelTool.annotations.idempotentHint, true);
  assert.equal(publicExecutionReadinessTool.annotations.readOnlyHint, true);
  assert.equal(publicExecutionReadinessTool.annotations.destructiveHint, false);
  assert.equal(publicExecutionReadinessTool.annotations.idempotentHint, true);
  const securitySentinel = await mcp('tools/call', { name: 'agent_security_sentinel_status', arguments: {} }, { key: keyA.key, name: 'agent_security_sentinel_status' });
  assert.equal(securitySentinel.body.result.structuredContent.guarantees.monitorOnly, true);
  assert.equal(securitySentinel.body.result.structuredContent.guarantees.storesCredentials, false);
  const publicExecutionReadiness = await mcp('tools/call', { name: 'public_execution_launch_readiness', arguments: {} }, { key: keyA.key, name: 'public_execution_launch_readiness' });
  assert.equal(publicExecutionReadiness.body.result.structuredContent.decision, 'deny');
  assert.equal(publicExecutionReadiness.body.result.structuredContent.status, 'locked');
  assert.equal(publicExecutionReadiness.body.result.structuredContent.execution.attempted, false);
  assert.equal(publicExecutionReadiness.body.result.structuredContent.liveScopeUsed, false);

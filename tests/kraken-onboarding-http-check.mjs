...
const { user: mfaUser } = await auth.registerUser('mfa-fixture@example.com', 'fixture-password-only');
mfaUser.totpSecret = generateSecret(); mfaUser.totpEnabled = true; auth.persistUsers();
...
try {
  ...
  const reauthPath = '/api/exchange-connections/reauthenticate';
  assert.equal((await request(reauthPath, { cookie, body: { action: 'connect', password: 'wrong-fixture-password' } })).status, 403);
  const proof = await request(reauthPath, { cookie, body: { action: 'connect', password: 'fixture-password-only' } });
  assert.equal(proof.status, 200);
  const saved = await request('/api/exchange-connections/kraken/onboarding-commit', { cookie, body: { token: plan.data.token, confirmed: true, reauthToken: proof.data.token } });
  ...
  assert.equal((await request('/api/account/kraken', { method: 'DELETE', cookie })).status, 403);
  const disconnect = await request(reauthPath, { cookie, body: { action: 'disconnect', password: 'fixture-password-only' } });
  assert.equal(disconnect.status, 200);
  const removed = await request('/api/account/kraken', { method: 'DELETE', cookie, body: { reauthToken: disconnect.data.token } });
  assert.equal(removed.status, 200);
  assert.equal(JSON.parse(readFileSync(join(scratch, 'users.json'), 'utf8'))[0].krakenKey, undefined);
  const first = await request('/api/auth/login', { body: { email: mfaUser.email, password: 'fixture-password-only' } });
  assert.equal(first.data.totpRequired, true);
  const second = await request('/api/auth/login/totp', { cookie: first.headers.get('set-cookie').split(';')[0], body: { email: mfaUser.email, totpCode: generateSync({ secret: mfaUser.totpSecret }) } });
  assert.equal(second.status, 200);
  const mfaCookie = second.headers.get('set-cookie').split(';')[0];
  assert.equal((await request(reauthPath, { cookie: mfaCookie, body: { action: 'connect', password: 'fixture-password-only' } })).status, 403);
  const authenticated = await request(reauthPath, { cookie: mfaCookie, body: { action: 'connect', password: 'fixture-password-only', totpCode: generateSync({ secret: mfaUser.totpSecret }) } });
  assert.equal(authenticated.status, 200);
  assert.equal(authenticated.data.expiresInMs, 120000);

...
import { createPositionGuardian } from './position-guardian.js';
...
export async function openTrade({ symbol, side = 'long', size, entry = null, stop = null, target = null, reason = '', confidence = null, accountSize = null, maxRiskPct = null, accountId = DEFAULT_ACCOUNT_ID, trailStopPct = null, trailTpPct = null, dca = null }) {
  ...
  // A positive explicit paper entry is already the fill authority for this
  // simulation. Do not make agent execution depend on an unrelated quote
  // network round trip after the server has validated that entry for limits.
  const ticker = entry && entry > 0 ? { last: entry } : await getTicker(symbol);
  ...
  // A protective exit on the wrong side of the fill would immediately create a
  // fabricated win/loss when the management sweep next runs. Reject it at the
  // ledger boundary, rather than relying on the UI to get the direction right.
  if (stopPrice != null && (side === 'long' ? stopPrice >= fill : stopPrice <= fill)) {
    return { ok: false, error: `stop must be ${side === 'long' ? 'below' : 'above'} entry for a ${side} position` };
  }
  if (target != null && (side === 'long' ? target <= fill : target >= fill)) {
    return { ok: false, error: `target must be ${side === 'long' ? 'above' : 'below'} entry for a ${side} position` };
  }
  ...
  position.guardian = createPositionGuardian(position, position.openedAt);
  ...
  const journalEntry = {
    ...
    guardian: structuredClone(position.guardian),
  };
...
export async function closeTrade(positionId, exitPrice = null, accountId = DEFAULT_ACCOUNT_ID, exitReason = null, exitAuthority = null) {
  ...
  let fill = exitPrice;
  let resolvedAuthority = exitAuthority || (exitPrice ? 'internal-simulation' : null);
  if (!fill || fill <= 0) {
    const t = await getTicker(pos.symbol).catch(() => null);
    fill = t?.last;
    if (fill) resolvedAuthority ||= 'server-market';
    else { fill = pos.lastPrice; resolvedAuthority ||= 'server-last-observed'; }
  }
  ...
  if (j) {
    ...
    // A caller-supplied price is only legitimate for an internal deterministic
    // simulation (for example a tested management sweep). HTTP/MCP closes use
    // a fresh server quote and record that provenance for later review/export.
    j.exitAuthority = resolvedAuthority;
  }
  state.account.cash += pos.notional + pnl;
  state.positions = state.positions.filter(p => p.id !== positionId);
  save();
  return { ok: true, exitPrice: fill, exitAuthority: j?.exitAuthority || resolvedAuthority, pnl: Math.round(pnl * 100) / 100, pnlPct: Math.round(pnlPct * 100) / 100, result, accountId: state.accountId };
}

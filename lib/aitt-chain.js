...
const ALLOCATION_ABI = ['function totalAllocated() view returns(uint256)'];
...
export async function probeReleaseState(config, reservedBaseUnits = 0n, rpcUrl = 'https://l2-mainnet.iost.io') {
  ...
  let allocationsVerified = reserve === configuredReserve;
  for (const [key, amount] of Object.entries(expected)) {
    ...
    if (await vault.totalAllocated() !== amount || await token.balanceOf(address) !== amount) allocationsVerified = false;
  }
  const verified = floor === 800_000_000n * unit && supply >= floor && supply <= 1_000_000_000n * unit
    ...
    && reserve >= outstanding + BigInt(reservedBaseUnits) && allocationsVerified;

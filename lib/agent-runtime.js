...
function validStoredQuarantine(quarantine) {
  if (quarantine === undefined) return true; // v1 runtimes created before Incident Center
  if (!quarantine || typeof quarantine !== 'object' || typeof quarantine.active !== 'boolean'
      || !Array.isArray(quarantine.incidentRefs)
      || quarantine.incidentRefs.some((ref) => !/^inc_[a-f0-9-]{36}$/.test(ref || ''))
      || new Set(quarantine.incidentRefs).size !== quarantine.incidentRefs.length) return false;
  if (quarantine.active && (!quarantine.incidentRefs.length || !Number.isFinite(quarantine.appliedAt))) return false;
  return !quarantine.reasonCode || typeof quarantine.reasonCode === 'string';
}

function validStoredSupervisor(supervisor) {
  if (supervisor === undefined || supervisor === null) return true;
  return supervisor && supervisor.version === 1
    && Number.isInteger(supervisor.cadenceMs) && supervisor.cadenceMs >= 5_000 && supervisor.cadenceMs <= HEARTBEAT_INTERVAL_MS
    && Number.isFinite(supervisor.lastAckAt);
}
...
function validStoredRuntime(runtime) {
  return runtime && typeof runtime === 'object'
    ...
    && validStoredQuarantine(runtime.quarantine)
    && validStoredSupervisor(runtime.supervisor);
}
...
function publicRuntime(runtime, now = Date.now(), { replayed = false } = {}) {
  ...
  const quarantined = !!runtime.quarantine?.active;
  return {
    ...
    supervisor: runtime.supervisor ? {
      managed: true, version: runtime.supervisor.version, cadenceMs: runtime.supervisor.cadenceMs,
      lastAckAt: runtime.supervisor.lastAckAt, healthy: current.ageMs <= DEGRADED_AFTER_MS,
    } : { managed: false, version: null, cadenceMs: null, lastAckAt: null, healthy: false },
    execution: {
      newMissionExposureAllowed: current.status === 'ready' && !quarantined,
      existingPositionProtection: 'position-guardian',
      authorityExpanded: false,
    },
    quarantine: {
      active: quarantined,
      incidentRef: quarantined ? (runtime.quarantine.incidentRefs?.[0] || runtime.quarantine.incidentRef) : null,
      incidentCount: quarantined ? (runtime.quarantine.incidentRefs?.length || 1) : 0,
      reasonCode: quarantined ? runtime.quarantine.reasonCode : null,
      appliedAt: quarantined ? runtime.quarantine.appliedAt : null,
      releaseRequiresOwner: quarantined,
    },
...
function validateHeartbeat(input) {
  ...
  const supervisorVersion = input.supervisorVersion == null ? null : Number(input.supervisorVersion);
  const cadenceMs = input.cadenceMs == null ? null : Number(input.cadenceMs);
  if ((supervisorVersion === null) !== (cadenceMs === null)) throw new Error('supervisorVersion and cadenceMs must be supplied together');
  if (supervisorVersion !== null && supervisorVersion !== 1) throw new Error('unsupported supervisor version');
  if (cadenceMs !== null && (!Number.isInteger(cadenceMs) || cadenceMs < 5_000 || cadenceMs > HEARTBEAT_INTERVAL_MS)) {
  ...
  return {
    ...
    ...(supervisorVersion === null ? {} : { supervisorVersion, cadenceMs }),
  };
}
...
export function recordAgentHeartbeat({ ownerId, keyId, agentName, ...input }, now = Date.now()) {
  ...
  if (!runtime) {
    ...
    runtime = {
      ...
      checkpoint: null, events: [], supervisor: heartbeat.supervisorVersion ? {
        version: heartbeat.supervisorVersion, cadenceMs: heartbeat.cadenceMs, lastAckAt: now,
      } : null,
    };
    runtime.checkpoint = {
  ...
  runtime.supervisor = heartbeat.supervisorVersion ? {
    version: heartbeat.supervisorVersion, cadenceMs: heartbeat.cadenceMs, lastAckAt: now,
  } : null;
...
export function agentRuntimeStatus(ownerId, keyId, now = Date.now()) {
  ...
  return runtime ? publicRuntime(runtime, now) : {
    ...
    supervisor: { managed: false, version: null, cadenceMs: null, lastAckAt: null, healthy: false },
    execution: { newMissionExposureAllowed: true, existingPositionProtection: 'position-guardian', authorityExpanded: false },
    quarantine: { active: false, incidentRef: null, reasonCode: null, appliedAt: null, releaseRequiresOwner: false },
...
export function ownerRuntimeStatus(ownerId, now = Date.now()) {
  ...
  const counts = { total: runtimes.length, ready: 0, degraded: 0, offline: 0, draining: 0, quarantined: 0 };
  for (const runtime of runtimes) if (Object.hasOwn(counts, runtime.status)) counts[runtime.status] += 1;
  counts.quarantined = runtimes.filter((runtime) => runtime.quarantine?.active).length;
  return {
...
export function missionRuntimeGate({ ownerId, keyId, missionId }, now = Date.now()) {
  ...
  if (runtime.quarantine?.active) return { ok: false, monitored: true, reason: 'agent-runtime-quarantined' };
  const current = health(runtime, now);
...
// Private server-to-server inspection for the incident engine. The result is
// never returned directly by an API because it contains identity bindings.
export function runtimeIncidentInputs(now = Date.now()) {
  return store.runtimes.map((runtime) => {
    const current = health(runtime, now);
    return {
      ownerId: runtime.ownerId, keyId: runtime.keyId, runtimeRef: runtime.runtimeRef,
      name: runtime.agentName, status: current.status, heartbeatAgeMs: current.ageMs,
      lastHeartbeatAt: runtime.lastHeartbeatAt, quarantineActive: !!runtime.quarantine?.active,
    };
  });
}

export function applyRuntimeQuarantine(ownerId, keyId, { incidentRef, reasonCode }, now = Date.now()) {
  const runtime = store.runtimes.find((item) => item.ownerId === ownerId && item.keyId === keyId) || null;
  if (!runtime) throw new Error('runtime not found');
  const existingRefs = runtime.quarantine?.active
    ? [...new Set([...(runtime.quarantine.incidentRefs || []), runtime.quarantine.incidentRef].filter(Boolean))]
    : [];
  if (existingRefs.includes(incidentRef)) return publicRuntime(runtime, now);
  runtime.quarantine = {
    active: true, incidentRefs: [...existingRefs, clean(incidentRef, 40)],
    reasonCode: clean(reasonCode, 80), appliedAt: runtime.quarantine?.appliedAt || now,
  };
  addEvent(runtime, 'quarantined', now, { reasonCode: runtime.quarantine.reasonCode });
  save();
  return publicRuntime(runtime, now);
}

export function releaseRuntimeQuarantine(ownerId, keyId, incidentRef, now = Date.now()) {
  const runtime = store.runtimes.find((item) => item.ownerId === ownerId && item.keyId === keyId) || null;
  if (!runtime) throw new Error('runtime not found');
  if (!runtime.quarantine?.active) return publicRuntime(runtime, now);
  const refs = [...new Set([...(runtime.quarantine.incidentRefs || []), runtime.quarantine.incidentRef].filter(Boolean))];
  if (!refs.includes(incidentRef)) throw new Error('quarantine incident mismatch');
  if (health(runtime, now).status !== 'ready') throw new Error('runtime must be ready before quarantine release');
  const remaining = refs.filter((ref) => ref !== incidentRef);
  runtime.quarantine = { ...runtime.quarantine, active: remaining.length > 0, incidentRefs: remaining, incidentRef: undefined, releasedAt: remaining.length ? null : now };
  addEvent(runtime, 'quarantine-released', now);
  save();
  return publicRuntime(runtime, now);
}

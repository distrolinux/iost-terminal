...
import { createDurableKrakenLane, inspectKrakenRecovery } from './kraken-durable-lane.js';

let durable = null, durableRoot = null;
export function configureKrakenCoordination(root) {
  if (durableRoot !== null && durableRoot !== root) throw Error('Coordination directory cannot change');
  durable = createDurableKrakenLane(root); durableRoot = root;
}
...
const lanes = new Map();
export function inspectKrakenRequestRecovery(apiKey) {
  if (!durableRoot || typeof apiKey !== 'string' || !apiKey || apiKey.length > 4096) return { status: 'unavailable', releaseAllowed: false, retryAllowed: false, executionAuthorized: false };
  return inspectKrakenRecovery(durableRoot, createHash('sha256').update(apiKey).digest('hex'));
}
export async function coordinateKrakenRequest(apiKey, operation, now = Date.now, method = null) {
  ...
  const run = lane.tail.then(async () => {
    ...
    if (durable) return durable(id, operation, now, method);
    const time = now();

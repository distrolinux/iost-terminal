...
export function createKrakenOnboarding({ persist, verify = verifyKrakenConnection, vault = configuredCredentialVault, enabled = () => process.env.KRAKEN_READONLY_ONBOARDING_ENABLED === '1', now = Date.now, schedule = setTimeout, cancel = clearTimeout } = {}) {
  const plans = new Map(), pending = new Set();
  const fail = reasonCode => ({ ok: false, reasonCode, executionAuthority: 'none' });
  const remove = token => { const p = plans.get(token); if (p) cancel(p.timer); plans.delete(token); };
  const prune = () => { for (const [token, plan] of plans) if (plan.expiresAt <= now()) remove(token); };
  const available = () => { try { return enabled() && Boolean(vault().activeKeyId); } catch { return false; } };
  return {
    pendingCount() { return plans.size; },
    ...
    async preview(user, sessionId, input) {
      ...
      try {
        ...
        for (const [key, p] of plans) if (p.owner === user.id && p.sessionId === sessionId) remove(key);
        if (plans.size >= 100) return fail('onboarding-busy');
        const token = randomBytes(32).toString('hex'), expiresAt = now() + 120000;
        const timer = schedule(() => remove(token), 120000); timer?.unref?.();
        plans.set(token, { owner: user.id, sessionId, replacement, passwordState, expiresAt, timer });

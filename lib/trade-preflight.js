...
import { buildExecutionDataTrust } from './agent-data-trust.js';
...
export function buildPaperTradePreflight({
  ...
  portfolioRisk = null, dataTrust = null, executionReadiness = null,
  ...
  const resolvedDataTrust = dataTrust || buildExecutionDataTrust({ ticker, now });
  ...
  const checks = [
    ...
    { code: 'data-trust-authorized', pass: resolvedDataTrust.decision === 'allow' },
    ...
    { code: 'agent-execution-ready', pass: executionReadiness?.decision !== 'deny' },
  ];
  ...
  const reasonCode = firstFailure?.code === 'wallet-pact-authorized'
    ...
    : firstFailure?.code === 'mission-authorized'
      ...
      : firstFailure?.code === 'data-trust-authorized'
        ? clean(resolvedDataTrust.reasonCode, 120) || firstFailure.code
      ...
      : firstFailure?.code === 'agent-execution-ready'
        ? clean(executionReadiness?.reasonCode, 120) || firstFailure.code
  ...
  const stableDataTrust = {
    ...resolvedDataTrust,
    checkedAt: null,
    evidence: {
      ...resolvedDataTrust.evidence,
      sources: Array.isArray(resolvedDataTrust.evidence?.sources) ? resolvedDataTrust.evidence.sources.map((source) => ({
        ...source, ageMs: null,
      })) : [],
    },
  };
  const stableExecutionReadiness = executionReadiness ? {
    ...executionReadiness,
    checkedAt: null,
    evidence: {
      ...executionReadiness.evidence,
      incidents: {
        ...executionReadiness.evidence?.incidents,
        recoveryAgeMs: null,
      },
    },
  } : null;
  ...
  const fingerprintPayload = {
    ...
    request: {
      ...
      maxSlippageBps: round(maxSlippageBps, 2), confidence: round(order.confidence, 2),
    ...
    dataTrust: stableDataTrust,
    executionReadiness: stableExecutionReadiness,
    checks,
  };
  const fingerprintInput = `iost-terminal:paper-preflight:v3:${stableStringify(fingerprintPayload)}`;
  ...
  return {
    ...
    request: {
      ...
      maxSlippageBps: round(maxSlippageBps, 2), confidence: round(order.confidence, 2), missionAttached: missionRequired,
    },
    market,
    costs: {
    ...
    dataTrust: resolvedDataTrust,
    executionReadiness: executionReadiness || {
      ok: true, mode: 'paper-only', readOnly: true, decision: 'allow',
      reasonCode: 'execution-readiness-not-provided', checks: [],
      decisionEffects: { newExposureAllowed: true, existingPositionsRemainProtected: true,

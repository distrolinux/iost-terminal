 ...
 */
export function checkPactSpend({ pactId, amountMinor, recipient = null, protocol = null }) {
  const p = getPact(pactId);
  ...
  if (p.policies?.limits?.maxPerTxMinor > 0 && amountMinor > p.policies.limits.maxPerTxMinor) {
    return { ok: false, reason: 'per-tx-cap', message: 'Exceeds pact per-transaction cap.' };
  }
  if (p.completion?.type === 'budget' && p.completion.budgetMinor > 0 && p.spentMinor + amountMinor > p.completion.budgetMinor) {

...
import { buildMultiVenueExecutionQuote } from './quote-integrity.js';
import { buildExecutionQualityDecision, createVenueHealthTracker } from './execution-quality.js';

const CACHE_TTL = { ticker: 10_000, klines: 45_000, trades: 15_000 };
const EXECUTION_QUOTE_TIMEOUT_MS = 2_500;

const cache = new Map();
const venueHealth = createVenueHealthTracker();
...
async function executionVenueQuote(symbol, source, fetcher) {
  const key = `execution-quote:${source}:${symbol}`;
  const hit = getCache(key, CACHE_TTL.ticker);
  if (hit) return { ...hit, venueHealth: venueHealth.snapshot(source) };
  ...
  try {
    ...
    const quote = {
      ...
      observedAt,
      latencyMs,
      venueHealth: venueHealth.record(source, { ok: true, latencyMs, now: observedAt }),
    };
    setCache(key, quote);
    return quote;
  } catch (error) {
    venueHealth.record(source, { ok: false, latencyMs: Date.now() - startedAt, now: Date.now() });
    throw error;
  }
}

async function okxExecutionQuote(sym) {
  const d = await fetchJson(`https://www.okx.com/api/v5/market/ticker?instId=${okxInst(sym)}`, {
    timeout: EXECUTION_QUOTE_TIMEOUT_MS,
  });
  const t = d?.code === '0' ? d.data?.[0] : null;
  if (!t) throw new Error(`OKX execution quote ${sym}`);
  return {
    source: 'OKX', last: +t.last, ask: +t.askPx, bid: +t.bidPx,
    high24h: +t.high24h, low24h: +t.low24h,
  };
}

async function kuCoinExecutionQuote(sym) {
  const d = await fetchJson(`https://api.kucoin.com/api/v1/market/orderbook/level1?symbol=${okxInst(sym)}`, {
    timeout: EXECUTION_QUOTE_TIMEOUT_MS,
  });
  if (d?.code !== '200000' || !d.data) throw new Error(`KuCoin execution quote ${sym}`);
  return {
    source: 'KuCoin', last: +d.data.price,
    ask: +d.data.bestAsk || +d.data.price,
    bid: +d.data.bestBid || +d.data.price,
    high24h: null, low24h: null,
  };
}

async function gateExecutionQuote(sym) {
  const d = await fetchJson(`https://api.gateio.ws/api/v4/spot/tickers?currency_pair=${gatePair(sym)}`, {
    timeout: EXECUTION_QUOTE_TIMEOUT_MS,
  });
  const t = d?.[0];
  if (!t) throw new Error(`Gate execution quote ${sym}`);
  return {
    source: 'Gate', last: +t.last,
    ask: +t.lowest_ask || +t.last,
    bid: +t.highest_bid || +t.last,
    high24h: +t.high_24h, low24h: +t.low_24h,
  };
}

// Execution uses an independent, stricter market-data path. Crypto quotes are
// gathered concurrently and must reach a two-venue consensus before routing to
// the best trustworthy ask (long) or bid (short). Scanning keeps its existing
// fast primary/failover path.
export async function getExecutionTicker(symbol, side = 'long', now = Date.now()) {
  const sym = String(symbol || '').trim().toUpperCase();
  if (STOCKS.includes(sym)) {
    await getTicker(sym);
    const ticker = peekTicker(sym, now);
    const executionQuality = buildExecutionQualityDecision({
      side, venues: ticker ? [{ ...ticker, latencyMs: null }] : [],
    });
    executionQuality.required = false;
    return {
      ...ticker,
      quoteIntegrity: {
        required: false, quorumMet: true, minimumVenues: 1,
        quoteCount: ticker ? 1 : 0, trustedVenueCount: ticker ? 1 : 0,
        excludedVenueCount: 0, excludedVenues: [],
        consensusPrice: ticker?.last ?? null, maximumOutlierBps: null,
        maximumVenueSpreadBps: 100,
        maximumObservedDeviationBps: 0, routeVenue: ticker?.source || null,
        routeLatencyMs: null, executionSide: side === 'short' ? 'bid' : 'ask',
        executionQuality,
        venues: ticker ? [{
          source: ticker.source, bid: ticker.bid, ask: ticker.ask,
          observedAt: ticker.observedAt, ageMs: ticker.ageMs,
          latencyMs: null, consensusDeviationBps: 0,
          spreadBps: 0,
  ...
  const attempts = await Promise.all([
    executionVenueQuote(sym, 'OKX', okxExecutionQuote).catch(() => null),
    executionVenueQuote(sym, 'KuCoin', kuCoinExecutionQuote).catch(() => null),
    executionVenueQuote(sym, 'Gate', gateExecutionQuote).catch(() => null),
  ]);
  return buildMultiVenueExecutionQuote({
    symbol: sym, side, quotes: attempts.filter(Boolean), now,
  });
}

...
function recordData(expected, observation, evidence) {
  ...
  if (typeof expected.venueAccountBinding !== 'string' || !/^[a-f0-9]{64}$/.test(expected.venueAccountBinding)) throw Error('account');
  const e = { clientOrderId: id(expected.clientOrderId), venueOrderId: id(expected.venueOrderId), pair: id(expected.pair), side: expected.side, quantity: decimal(expected.quantity), credentialBinding: expected.credentialBinding, venueAccountBinding: expected.venueAccountBinding };

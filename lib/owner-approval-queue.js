...
export const ownerApprovalMissionRef = (missionId) => missionId ? ref('mission', missionId) : null;
...
function normalizeOrder(order = {}) {
  return {
    ...
    missionRef: ownerApprovalMissionRef(order.missionId),
...
function publicOrder(order) {
  return { symbol: order.symbol, side: order.side, size: order.size, entry: order.entry, stop: order.stop,
    ...
    missionAttached: !!order.missionRef, missionRef: order.missionRef || null };
}

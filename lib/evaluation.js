...
function baselines(candles, folds, costs) {
  let buy = 1; let smaValue = 1; let priorExposure = false;
  const turnoverCost = (costs.feeBps + costs.spreadBps / 2 + costs.slippageBps) / 10_000;
  for (const fold of folds) {
    buy *= candles[fold.test.toIndex].c / candles[fold.test.fromIndex].o * (1 - turnoverCost * 2);
    for (let i = fold.test.fromIndex; i < fold.test.toIndex; i++) {
  ...
  return { buyAndHold: { returnPct: round((buy - 1) * 100), description: 'long each out-of-sample fold, same modeled costs' },
    smaCross: { returnPct: round((smaValue - 1) * 100), description: '20/50 SMA long-or-cash, causal observations, same modeled costs' },
...
export function evaluateAgentStrategy({ symbol, timeframe = '1d', strategy, candles: input, config = {} }) {
  try {
    ...
    const core = { version: 1, mode: 'paper-only', symbol: String(symbol).toUpperCase(), timeframe, strategy: frozenStrategy, config: cfg,
      ...
      folds, metrics: summary, baselines: baselineResult, calibration: calibrationResult, warnings,
      audit: { ok: true, source: 'validated immutable candle snapshot', strategyHash: hash(frozenStrategy), dataHash: hash(candles) } };
    const promotion = promotionPolicy(core, { minimumTrades: cfg.minimumTrades });
    return { ok: true, ...core, promotion, evidence: { resultHash: hash({ ...core, promotion }) }, trades };
  } catch (error) {

...
// Signals see bars through index i; fills occur after a configured 1–5 bar delay. Strategy parameters
...
import { strategyPromotionPolicy } from './strategy-promotion.js';
import { createAgentBenchmark, verifyAgentBenchmark } from './agent-benchmark.js';
import { createAgentChallenge, verifyAgentChallenge } from './agent-challenge.js';
...
const DEFAULTS = Object.freeze({ trainBars: 120, testBars: 40, stepBars: 40, minimumTrades: 30, executionDelayBars: 1,
...
function normalizeConfig(config = {}) {
  ...
  const executionDelayBars = Number(config.executionDelayBars ?? DEFAULTS.executionDelayBars);
  if (!Number.isInteger(executionDelayBars) || executionDelayBars < 1 || executionDelayBars > 5) throw new Error('executionDelayBars must be an integer between 1 and 5');
  return { trainBars: integer('trainBars', DEFAULTS.trainBars, 40), testBars: integer('testBars', DEFAULTS.testBars, 10),
    stepBars: integer('stepBars', DEFAULTS.stepBars, 10), minimumTrades: integer('minimumTrades', DEFAULTS.minimumTrades, 1), executionDelayBars, costs };
}
...
function runFold({ candles, strategy, fold, costs, startingEquity, executionDelayBars = 1 }) {
...
export function promotionPolicy(evaluation, policy = {}) {
  return strategyPromotionPolicy(evaluation, policy);
}

export function evaluateAgentStrategy({ symbol, timeframe = '1d', strategy, candles: input, config = {}, _skipChallenge = false }) {
  try {
    ...
    for (const fold of folds) {
      ...
      const result = runFold({ candles, strategy: frozenStrategy, fold, costs: cfg.costs, startingEquity: equity, executionDelayBars: cfg.executionDelayBars });
    ...
    const core = { version: 2, mode: 'paper-only', symbol: normalizedSymbol, timeframe, strategy: frozenStrategy, config: cfg,
      methodology: { split: 'rolling walk-forward', parameters: 'frozen before evaluation', execution: `${cfg.executionDelayBars}-bar-delayed open`,
    ...
    const nominalBenchmark = createAgentBenchmark({ ...core, promotion, challenge: null }, trades);
    const nominalCore = { ...core, promotion, challenge: null, benchmark: nominalBenchmark, trades };
    const nominal = { ok: true, ...nominalCore, evidence: { resultHash: evidenceHash(nominalCore) } };
    const shock = (id, label, nextConfig) => ({ id, label, assumptions: nextConfig,
      evaluation: evaluateAgentStrategy({ symbol: normalizedSymbol, timeframe, strategy: frozenStrategy, candles, config: nextConfig, _skipChallenge: true }) });
    const fee = Math.min(500, Math.max(cfg.costs.feeBps * 2, cfg.costs.feeBps + 10));
    const spread = Math.min(500, Math.max(cfg.costs.spreadBps * 3, cfg.costs.spreadBps + 20));
    const slippage = Math.min(500, Math.max(cfg.costs.slippageBps * 3, cfg.costs.slippageBps + 20));
    const stressRuns = _skipChallenge ? [] : [
      shock('fee-shock', 'Fee shock', { ...cfg, costs: { ...cfg.costs, feeBps: fee } }),
      shock('thin-book', 'Thin-book costs', { ...cfg, costs: { ...cfg.costs, spreadBps: spread, slippageBps: slippage } }),
      shock('delayed-fill', 'Delayed fill', { ...cfg, executionDelayBars: Math.min(5, cfg.executionDelayBars + 1) }),
      shock('combined-stress', 'Combined stress', { ...cfg, executionDelayBars: Math.min(5, cfg.executionDelayBars + 1), costs: { feeBps: fee, spreadBps: spread, slippageBps: slippage } }),
    ];
    const challenge = _skipChallenge ? null : createAgentChallenge(nominal, stressRuns);
    const benchmark = createAgentBenchmark({ ...core, promotion, challenge }, trades);
    return { ok: true, ...core, promotion, challenge, benchmark, evidence: { resultHash: evidenceHash({ ...core, promotion, challenge, benchmark, trades }) }, trades };
  } catch (error) {
...
export function verifyEvaluationEvidence(evaluation) {
  ...
  if (evaluation.challenge && !verifyAgentChallenge(evaluation.challenge)) return false;
  if (evaluation.benchmark && !verifyAgentBenchmark(evaluation)) return false;

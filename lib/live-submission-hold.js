...
export function createLiveSubmissionHold(directory) {
  ...
  return {
    ...
    claim(ownerId, order, credentialBinding) {
      try {
        ...
        try {
          writeFileSync(fd, JSON.stringify({ version: 1, clientOrderId, credentialBinding, status: 'submission-unreconciled', order: { symbol: order.symbol, side: order.side, size: String(order.size), entry: order.entry == null ? null : String(order.entry) }, createdAt: Date.now() }));
          fsyncSync(fd);
        } finally { closeSync(fd); }
        const dir = openSync(directory, 'r');
        try { fsyncSync(dir); } finally { closeSync(dir); }

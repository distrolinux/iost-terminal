...
export function createLiveSubmissionHold(directory) {
  ...
  return {
    ...
    claim(ownerId, order, credentialBinding, venueAccountBinding) {
      try {
        if (typeof credentialBinding !== 'string' || !/^[a-f0-9]{64}$/.test(credentialBinding)) throw Error('connection');
        if (typeof venueAccountBinding !== 'string' || !/^[a-f0-9]{64}$/.test(venueAccountBinding)) throw Error('account');
        if (typeof ownerId !== 'string' || !ownerId || ownerId.length > 256) throw Error('owner');
        ...
        try {
          writeFileSync(fd, JSON.stringify({ version: 1, clientOrderId, credentialBinding, venueAccountBinding, status: 'submission-unreconciled', order: { symbol: order.symbol, side: order.side, size: String(order.size), entry: order.entry == null ? null : String(order.entry) }, createdAt: Date.now() }));
          fsyncSync(fd);
        } finally { closeSync(fd); }
        const dir = openSync(directory, 'r');
        try { fsyncSync(dir); } finally { closeSync(dir); }

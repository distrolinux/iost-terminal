...
export function buildOwnerActionWorkbench({
  ...
  const dataTrustReady = dataTrust.decision === 'allow' || dataTrust.status === 'healthy';
  const coverage = guardian.coverage || {};
  const activePaperWallets = wallets.filter((wallet) => wallet.status === 'active'
    && wallet.capabilities?.includes('trade.paper'));
  // A user can own several active paper wallets. Select the wallet that forms
  // the authoritative active Pact pair instead of assuming the first wallet is
  // the delegated one.
  const activePact = authorization ? pacts.find((pact) => pact.status === 'active'
    && activePaperWallets.some((wallet) => wallet.walletId === pact.agentWalletId)) || null : null;
  const activeWallet = (activePact
    ? activePaperWallets.find((wallet) => wallet.walletId === activePact.agentWalletId)
    : activePaperWallets[0]) || null;
  ...
  if (!dataTrustReady) actions.push(item('data-trust', 'Restore trusted market evidence',

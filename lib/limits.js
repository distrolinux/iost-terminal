...
/** Read-only spend check. It neither creates a usage row nor rolls/persists a window. */
export function previewSpend({ walletId, amountMinor, purpose = '' }) {
  const w = getWallet(walletId);
  if (!w) return { ok: false, reason: 'no-wallet', message: 'No wallet for this principal.' };
  if (w.status !== 'active') return { ok: false, reason: 'wallet-suspended', message: 'Wallet is suspended.' };
  if (isFrozen()) return { ok: false, reason: 'frozen', message: 'Agent operations are frozen.' };
  ...
  const lim = w.limits?.USD || {};
  const maxPerTx = lim.maxPerTxMinor || 0;
  if (maxPerTx > 0 && amt > maxPerTx) return { ok: false, reason: 'per-tx-cap', message: 'Exceeds per-transaction cap.' };
  const row = store.usage[walletId];
  const dayUsed = row?.day?.key === utcDayKey() ? Number(row.day.used) || 0 : 0;
  const weekUsed = row?.week?.key === utcWeekKey() ? Number(row.week.used) || 0 : 0;
  const remDaily = (lim.dailyCapMinor || 0) > 0 ? Math.max(0, lim.dailyCapMinor - dayUsed) : Infinity;
  const remWeekly = (lim.weeklyCapMinor || 0) > 0 ? Math.max(0, lim.weeklyCapMinor - weekUsed) : Infinity;
  if (remDaily !== Infinity && amt > remDaily) return { ok: false, reason: 'daily-cap', message: 'Exceeds remaining daily cap.' };
  if (remWeekly !== Infinity && amt > remWeekly) return { ok: false, reason: 'weekly-cap', message: 'Exceeds remaining weekly cap.' };
  return { ok: true, remainingDailyMinor: remDaily === Infinity ? -1 : remDaily, remainingWeeklyMinor: remWeekly === Infinity ? -1 : remWeekly, purpose };
}

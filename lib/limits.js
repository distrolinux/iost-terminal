 ...
 */
export function reserveSpend({ walletId, amountMinor, purpose = '' }) {
  ...
  row.reserved.push({ reserveId, amount: amt, dayKey: row.day.key, weekKey: row.week.key });
  save();
...
export function releaseReserve({ walletId, reserveId }) {
  ...
  save();
  return { ok: true, released: r.amount };
}
...
export function commitReserve({ walletId, reserveId }) {
  ...
  save();
  return { ok: true, amount: settled.amount };
}

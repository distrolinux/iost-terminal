...
import { listAccounts, getState, closeTrade, addToPosition, markToMarket } from './paper.js';
...
export async function sweepManagement({ priceFn = null, scoreFn = null } = {}) {
  ...
  for (const acc of listAccounts()) {
    ...
    for (const pos of state.positions.slice()) {
      ...
      if (tr.exit) {
        await closeTrade(pos.id, priceNow, acc.accountId, tr.reason);
...
export { trailLevels, checkTrailing, checkDca };

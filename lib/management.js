...
export async function sweepManagement({ priceFn = null, scoreFn = null } = {}) {
  ...
  for (const acc of listAccounts()) {
    ...
    for (const pos of state.positions.slice()) {
      ...
      if (fixed.exit) {
        await closeTrade(pos.id, priceNow, acc.accountId, fixed.reason, 'management-observed-price');
      ...
      if (tr.exit) {
        await closeTrade(pos.id, priceNow, acc.accountId, tr.reason, 'management-observed-price');

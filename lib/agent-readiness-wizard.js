...
export function buildAgentReadinessWizard({ wallets = [], pacts = [], keys = [], runtime = {}, missions = [],
  ...
  const runningMission = wallet && pact ? missions.find((item) => item.status === 'running'
    && item.agentWalletId === wallet.walletId && item.pactId === pact.pactId) : null;
  ...
  const burnClear = safetySlo.status !== 'budget-exhausted'
    && !safetySlo.burnRates?.fast?.firing && !safetySlo.burnRates?.slow?.firing;

...
import { readKrakenLedgerWindow } from '../kraken-ledger-window.js';
import { readLinkedKrakenSettlement } from '../kraken-linked-settlement.js';
import { assessKrakenPermissions } from '../kraken-connection-verification.js';
import { previewLiveReconciliation } from '../live-reconciliation-preview.js';
import { coordinateKrakenRequest, inspectKrakenRequestRecovery } from '../kraken-request-coordinator.js';
...
export function createKrakenBroker({ apiKey: keyOverride, secret: secretOverride, apiSecret, ownerId: boundOwnerId } = {}) {
  ...
  async function privateCall(method, params = {}) {
    ...
    return coordinateKrakenRequest(key, async nonce => {
  ...
  async function getVenueIdentity(ownerId) {
    try {
      if (!credentialBinding(ownerId)) throw Error('owner');
      const info = await privateCall('GetApiKeyInfo');
      if (info.apiKey !== key || typeof info.iban !== 'string' || !/^[A-Za-z0-9 ]{8,128}$/.test(info.iban) || !info.iban.trim()) throw Error('identity');
      if (typeof info.validUntil !== 'string' || !/^\d{1,16}$/.test(info.validUntil)) throw Error('expiry');
      const expiry = BigInt(info.validUntil);
      if (expiry !== 0n && expiry <= BigInt(Math.floor(Date.now() / 1000))) throw Error('expired');
      if (assessKrakenPermissions(info).permissionStatus !== 'observed-allowed-permissions' || !['query-open-trades', 'query-closed-trades'].every(p => info.permissions.includes(p))) throw Error('permissions');
      const venueAccountBinding = createHmac('sha256', secret).update(JSON.stringify(['iost-kraken-account-v1', ownerId, key, info.iban])).digest('hex');
      return { ok: true, venueAccountBinding, ledgerPermission: info.permissions.includes('query-ledger'), submissionPermissions: ['modify-trades', 'close-trades'].every(p => info.permissions.includes(p)) };
  ...
  return {
    ...
    credentialBinding,
    getVenueIdentity,
    async getLinkedSettlementEvidence(hold, ownerId, window) {
      const held = { status: 'unknown', settlementVerified: false, releaseAllowed: false, executionAuthorized: false };
      try {
        if (!credentialBinding(ownerId) || hold?.credentialBinding !== credentialBinding(ownerId) || hold?.order?.symbol !== 'BTC') return held;
        const identity = await getVenueIdentity(ownerId);
        if (!identity.ok || !identity.ledgerPermission || identity.venueAccountBinding !== hold.venueAccountBinding) return held;
        const evidence = await this.getOrderEvidence(hold, ownerId);
        if (!evidence.ok || previewLiveReconciliation(evidence.expected, evidence.observation).status === 'unknown') return held;
        return await readLinkedKrakenSettlement(privateCall, evidence.fillSummary, window);
      } catch { return held; }
    },
    // Internal owner-bound evidence only; never expose raw ledger rows publicly.
    async getLedgerWindow(hold, ownerId, window) {
      ...
      try {
        ...
        if (!binding || hold?.credentialBinding !== binding || typeof hold.venueAccountBinding !== 'string') return unknown;
        const identity = await getVenueIdentity(ownerId);
        if (!identity.ok || !identity.ledgerPermission || identity.venueAccountBinding !== hold.venueAccountBinding) return unknown;
        return await readKrakenLedgerWindow(privateCall, window);
      } catch { return unknown; }
    },
    getRecoveryStatus(ownerId) {
      if (!credentialBinding(ownerId)) return { status: 'unavailable', releaseAllowed: false, retryAllowed: false, executionAuthorized: false };
      return inspectKrakenRequestRecovery(key);
    },

    // Internal read-only recovery using a separately provisioned credential.
    // Original key lane/hold is never touched. No credential storage or routes.
    async readRecoveryOrder(hold, ownerId, recovery) {
      const unknown = { status: 'unknown', releaseAllowed: false, retryAllowed: false, executionAuthorized: false };
      try {
        const binding = credentialBinding(ownerId);
        if (!binding || hold?.credentialBinding !== binding || typeof hold.venueAccountBinding !== 'string' || !/^[a-f0-9]{64}$/.test(hold.venueAccountBinding)) return unknown;
        if (typeof recovery?.apiKey !== 'string' || !recovery.apiKey || recovery.apiKey === key || typeof recovery.apiSecret !== 'string' || !recovery.apiSecret) return unknown;
        const pair = PAIR_OF[hold.order?.symbol];
        if (!pair || !['long', 'short'].includes(hold.order.side) || typeof hold.venueOrderId !== 'string' || !/^[A-Za-z0-9-]{1,80}$/.test(hold.venueOrderId)) return unknown;
        const read = (method, params = {}) => {
          if (!['GetApiKeyInfo', 'QueryOrders'].includes(method)) throw Error('read-only recovery');
          return coordinateKrakenRequest(recovery.apiKey, async nonce => {
            const path = `/0/private/${method}`;
            const body = new URLSearchParams({ nonce, ...params }).toString();
            const res = await jsonFetch(API_BASE + path, { method: 'POST', headers: { 'API-Key': recovery.apiKey, 'API-Sign': sign(recovery.apiSecret, path, body, nonce), 'Content-Type': 'application/x-www-form-urlencoded' }, body });
            if (!Array.isArray(res.error) || res.error.length || !res.result || typeof res.result !== 'object' || Array.isArray(res.result)) throw Error('recovery unavailable');
            return res.result;
          }, Date.now, method);
        };
        const info = await read('GetApiKeyInfo');
        if (info.apiKey !== recovery.apiKey || typeof info.iban !== 'string' || !/^[A-Za-z0-9 ]{8,128}$/.test(info.iban) || !info.iban.trim()) return unknown;
        if (typeof info.validUntil !== 'string' || !/^\d{1,16}$/.test(info.validUntil) || (BigInt(info.validUntil) !== 0n && BigInt(info.validUntil) <= BigInt(Math.floor(Date.now() / 1000)))) return unknown;
        const permissions = assessKrakenPermissions(info);
        if (permissions.permissionStatus !== 'observed-allowed-permissions' || permissions.profile !== 'read-only' || !['query-open-trades', 'query-closed-trades'].every(p => info.permissions.includes(p))) return unknown;
        const account = createHmac('sha256', secret).update(JSON.stringify(['iost-kraken-account-v1', ownerId, key, info.iban])).digest('hex');
        if (account !== hold.venueAccountBinding) return unknown;
        const orders = await read('QueryOrders', { txid: hold.venueOrderId, trades: 'false' });
        if (Object.keys(orders).length !== 1 || !Object.hasOwn(orders, hold.venueOrderId)) return unknown;
        const o = orders[hold.venueOrderId];
        const decimal = value => {
          if (typeof value !== 'string' || !/^(0|[1-9]\d{0,19})(\.\d{1,10})?$/.test(value)) throw Error('decimal');
          const [w, f = ''] = value.split('.'); return BigInt(w) * 10000000000n + BigInt(f.padEnd(10, '0'));
        };
        const type = hold.order.entry === null ? 'market' : 'limit';
        if (o?.descr?.ordertype !== type || (type === 'limit' && decimal(o.descr.price) !== decimal(hold.order.entry))) return unknown;
        const expected = { clientOrderId: hold.clientOrderId, venueOrderId: hold.venueOrderId, pair, side: hold.order.side === 'short' ? 'sell' : 'buy', quantity: hold.order.size };
        ...
        return { ...unknown, ...previewLiveReconciliation(expected, observation), recoveryReadOnly: true, settlementVerified: false };
      } catch { return unknown; }
    },
    ...
    async getOrderEvidence(hold, ownerId) {
      try {
        ...
        if (typeof hold.venueAccountBinding !== 'string' || !/^[a-f0-9]{64}$/.test(hold.venueAccountBinding)) throw Error('account unavailable');
        const identity = await getVenueIdentity(ownerId);
        if (!identity.ok || identity.venueAccountBinding !== hold.venueAccountBinding) throw Error('account mismatch');
        const pair = PAIR_OF[hold?.order?.symbol];
        ...
        const expected = { clientOrderId: hold.clientOrderId, venueOrderId: hold.venueOrderId, pair, side: hold.order.side === 'short' ? 'sell' : 'buy', quantity: hold.order.size, credentialBinding: binding, venueAccountBinding: identity.venueAccountBinding };
        const observation = { clientOrderId: o.cl_ord_id, venueOrderId: hold.venueOrderId, pair: RESPONSE_KEYS[o.descr.pair] || o.descr.pair, side: o.descr.type, quantity: o.vol, filledQuantity: fillQuantity(o), status: o.status };

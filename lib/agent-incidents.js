...
function publicIncident(incident) {
  return {
    ...
    ownerReviewRequired: (incident.quarantineApplied || incident.category === 'runtime-recovery-failure')
...
export function reconcileRuntimeIncidents(runtimeInputs = agentRuntime.runtimeIncidentInputs(), now = Date.now()) {
  ...
  for (const runtime of runtimeInputs) {
    ...
    // A rejected recovery attempt is no longer an active runtime symptom once
    // the supervised runtime is healthy. Quarantined failures always require
    // release; an acknowledged warning becomes recovery-ready immediately so
    // the owner can finish review without waiting for automatic expiry.
    if (recoveryFailure && runtime.status === 'ready' && recoveryFailure.status !== 'recovery-detected'
        && (recoveryFailure.quarantineApplied || recoveryFailure.acknowledgedAt)) {

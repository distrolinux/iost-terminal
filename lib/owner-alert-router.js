...
function loadStore() {
  if (!existsSync(FILE)) return { version: OWNER_ALERT_ROUTER_VERSION, alerts: [], receipts: [], sloSignals: {} };
  let parsed;
...
function incidentSignal(ownerId, incident) {
  const status = incident.status === 'recovery-detected' ? 'recovery-ready' : incident.status;
  return {

...
import { getPact, previewPact } from './pacts.js';
...
export function checkMissionTrade({ missionId: id, ownerId, walletId, pactId, symbol, notionalMinor, approvalAuthorized = false }) {
  const mission = ownedMission(id, ownerId);
  ...
  if (mission.approvalMode !== 'within-pact' && !approvalAuthorized) return { ok: false, reason: 'mission-approval-required', message: 'This mission requires an exact owner approval before paper execution.' };
...
/** Mission authorization preview without expiry or reservation persistence. */
export function previewMissionTrade({ missionId: id, ownerId, walletId, pactId, symbol, notionalMinor }, now = Date.now()) {
  const mission = store.missions.find((item) => item.missionId === id && item.ownerId === ownerId) || null;
  if (!mission) return { ok: false, reason: 'mission-not-found', message: 'Mission not found.' };
  const status = ['running', 'paused'].includes(mission.status) && now >= mission.expiresAt ? 'expired' : mission.status;
  if (status !== 'running') return { ok: false, reason: `mission-${status}`, message: `Mission is ${status}.` };
  const wallet = getWallet(mission.walletId);
  const pact = previewPact(mission.pactId, now);
  const authorityValid = wallet?.ownerId === ownerId && wallet?.kind === 'agent' && wallet?.status === 'active'
    && wallet?.capabilities?.includes('trade.paper') && !wallet?.capabilities?.includes('trade.live')
    && pact?.ownerId === ownerId && pact?.agentWalletId === wallet.walletId && pact?.status === 'active';
  if (!authorityValid) return { ok: false, reason: 'mission-authorization-invalid', message: 'Mission authority is not currently active.' };
  if (mission.walletId !== walletId || mission.pactId !== pactId) return { ok: false, reason: 'mission-authority-mismatch', message: 'Order authority does not match the mission.' };
  if (!mission.symbols.includes(String(symbol || '').toUpperCase())) return { ok: false, reason: 'symbol-not-allowed', message: 'Symbol is outside the mission watchlist.' };
  const amount = Number(notionalMinor);
  if (!Number.isSafeInteger(amount) || amount <= 0) return { ok: false, reason: 'mission-invalid-notional', message: 'Mission order notional must be positive.' };
  if (amount > mission.maxOrderMinor) return { ok: false, reason: 'mission-order-cap', message: 'Order exceeds the mission maximum.' };
  const activeReservations = (mission.reservations || []).filter((reservation) => now - reservation.createdAt < RESERVATION_TTL_MS);
  if (mission.tradesOpened + activeReservations.length >= mission.maxTrades) return { ok: false, reason: 'mission-trade-cap', message: 'Mission trade count is exhausted.' };
  if (mission.realizedPnlMinor <= -mission.maxLossMinor) return { ok: false, reason: 'mission-loss-halt', message: 'Mission loss limit is reached.' };
  return { ok: true, approvalMode: mission.approvalMode, approvalRequired: mission.approvalMode !== 'within-pact' };
}

export function missionApprovalRequirement(id, ownerId, { notionalMinor = 0, confidence = null, warnings = [], volatilityRegime = null } = {}) {
  const mission = store.missions.find((item) => item.missionId === id && item.ownerId === ownerId) || null;
  if (!mission) return { required: false, mode: null, reasons: [] };
  if (mission.approvalMode === 'within-pact') return { required: false, mode: 'within-pact', reasons: [] };
  if (mission.approvalMode === 'per-order') return { required: true, mode: 'per-order', reasons: ['per-order-policy'] };
  const reasons = [];
  if (Number(notionalMinor) >= Math.ceil(mission.maxOrderMinor / 2)) reasons.push('large-order-exception');
  if (!Number.isFinite(Number(confidence)) || Number(confidence) < 70) reasons.push('low-confidence-exception');
  if (Array.isArray(warnings) && warnings.length) reasons.push('preflight-warning-exception');
  if (!['calm', 'normal'].includes(volatilityRegime)) reasons.push('volatility-exception');
  return { required: reasons.length > 0, mode: 'exceptions', reasons };
}

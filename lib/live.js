...
const AUDIT = join(ROOT, 'data', 'live-audit.jsonl');
...
/** Enable live mode for an account. `ownKeys` = user has their own venue key
 * connected (v3) — that bypasses the email allowlist (their own money, their
 * own venue account). Platform venue (owner's .env key) stays allowlist-only. */
export async function enableLive(state, ownerEmail, ownKeys = false) {
  if (liveOf(state).enabled) return { ok: false, error: 'live mode already enabled' };
  if (!ownKeys && !isLiveAllowed(ownerEmail)) return { ok: false, error: 'email not on live-trading allowlist' };
  if (!liveConfigReady()) return { ok: false, error: 'venue keys not configured (KRAKEN_API_KEY/SECRET missing)' };
  state.live = { enabled: true, venue: ownKeys ? 'kraken:self' : 'kraken', enabledAt: Date.now(), pilot: true };
...
export function getLiveState(state, ownerEmail) {
  ...
  return {
    ...
    krakenConfigured: liveConfigReady(),
  };
}

...
const PAPER_TRADE_TOOLS = [
  readTool('paper_trade_preflight', 'Preflight paper trade', 'Read-only paper execution preflight with deterministic data-trust provenance, multi-venue quote integrity, price-protected execution-quality scoring and venue failover, plus server-authoritative portfolio exposure, concentration, correlation, drawdown, daily-loss, protective-stop, volatility fallback and dynamic risk-capacity gates.', objectSchema({
    intentId: { type: 'string', minLength: 8, maxLength: 128, pattern: '^[A-Za-z0-9][A-Za-z0-9._:-]{7,127}$' },
    symbol: { type: 'string', minLength: 1, maxLength: 24, pattern: '^[A-Za-z0-9._/-]+$' },
    side: { type: 'string', enum: ['long', 'short'] },
  ...
  mutationTool('paper_trade_open', 'Open paper trade', 'Open one idempotent simulated position using its unexpired preflight fingerprint only after data trust, quote integrity, price-protected execution quality and server-authoritative portfolio risk pass. Agent opens require a protective stop and fill at the bound quality-selected server ask or bid.', objectSchema({
    intentId: { type: 'string', minLength: 8, maxLength: 128, pattern: '^[A-Za-z0-9][A-Za-z0-9._:-]{7,127}$' },

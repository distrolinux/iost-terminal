...
const PAPER_TRADE_TOOLS = [
  readTool('paper_trade_preflight', 'Preflight paper trade', 'Read-only paper execution preflight with multi-venue quote integrity plus server-authoritative portfolio exposure, concentration, correlation, drawdown, daily-loss, protective-stop, volatility fallback and dynamic risk-capacity gates.', objectSchema({
    intentId: { type: 'string', minLength: 8, maxLength: 128, pattern: '^[A-Za-z0-9][A-Za-z0-9._:-]{7,127}$' },
    symbol: { type: 'string', minLength: 1, maxLength: 24, pattern: '^[A-Za-z0-9._/-]+$' },
    side: { type: 'string', enum: ['long', 'short'] },

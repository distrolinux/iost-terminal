...
const PAPER_TRADE_TOOLS = [
  readTool('paper_trade_preflight', 'Preflight paper trade', 'Read-only paper execution preflight with fresh quote evidence, estimated costs, cash, wallet/Pact, spend-limit and mission checks. It never reserves funds or places a trade.', objectSchema({
    intentId: { type: 'string', minLength: 8, maxLength: 128, pattern: '^[A-Za-z0-9][A-Za-z0-9._:-]{7,127}$' },
    symbol: { type: 'string', minLength: 1, maxLength: 24, pattern: '^[A-Za-z0-9._/-]+$' },
    side: { type: 'string', enum: ['long', 'short'] },
  ...
  }, ['intentId', 'symbol', 'side', 'size', 'entry', 'walletId', 'pactId'])),
  mutationTool('paper_trade_open', 'Open paper trade', 'Open a simulated position bound to an unexpired preflight fingerprint and one execution intent. Requires trade-paper scope, positive trusted entry and size, an owned agent wallet and an active wallet-bound Pact.', objectSchema({
    intentId: { type: 'string', minLength: 8, maxLength: 128, pattern: '^[A-Za-z0-9][A-Za-z0-9._:-]{7,127}$' },
    ...
    recipient: { type: ['string', 'null'], maxLength: 256 },
    protocol: { type: ['string', 'null'], maxLength: 128 },
  }, ['intentId', 'preflightFingerprint', 'symbol', 'side', 'size', 'entry', 'walletId', 'pactId']), { idempotent: true }),

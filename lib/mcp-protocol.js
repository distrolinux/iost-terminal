...
const PAPER_TRADE_TOOLS = [
  readTool('paper_trade_preflight', 'Preflight paper trade', 'Read-only paper execution preflight with multi-venue quote quorum, stale/outlier rejection, best trustworthy bid/ask routing, hard spread and slippage caps, server-fill notional, wallet/Pact and mission checks.', objectSchema({
    intentId: { type: 'string', minLength: 8, maxLength: 128, pattern: '^[A-Za-z0-9][A-Za-z0-9._:-]{7,127}$' },
    symbol: { type: 'string', minLength: 1, maxLength: 24, pattern: '^[A-Za-z0-9._/-]+$' },
    side: { type: 'string', enum: ['long', 'short'] },
  ...
  mutationTool('paper_trade_open', 'Open paper trade', 'Open one idempotent simulated position at the best fresh consensus-approved server ask for long or bid for short. Requires an unexpired preflight fingerprint, explicit slippage tolerance, trade-paper scope, owned wallet and active wallet-bound Pact.', objectSchema({
    intentId: { type: 'string', minLength: 8, maxLength: 128, pattern: '^[A-Za-z0-9][A-Za-z0-9._:-]{7,127}$' },

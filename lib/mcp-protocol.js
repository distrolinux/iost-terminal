...
const mutationTool = (name, title, description, inputSchema, { destructive = false } = {}) => ({
  name, title, description, inputSchema, outputSchema: outputObject,
...
const PAPER_TRADE_TOOLS = [
  mutationTool('paper_trade_open', 'Open paper trade', 'Open a simulated position. Requires trade-paper scope, positive trusted entry and size, an owned agent wallet and an active wallet-bound Pact.', objectSchema({
    symbol: { type: 'string', minLength: 1, maxLength: 24, pattern: '^[A-Za-z0-9._/-]+$' },
    side: { type: 'string', enum: ['long', 'short'] },
  ...
  }, ['symbol', 'side', 'size', 'entry', 'walletId', 'pactId'])),
  mutationTool('paper_trade_close', 'Close paper trade', 'Close one position in the authenticated user’s simulated account.', objectSchema({
    positionId: { type: 'string', minLength: 1, maxLength: 128 },
    exitPrice: { type: ['number', 'null'], exclusiveMinimum: 0 },
  }, ['positionId']), { destructive: true }),
];

export function buildMcpTools({ authenticated = false, scopes = [] } = {}) {
  ...
  return tools.map((tool) => structuredClone(tool)).sort((a, b) => a.name.localeCompare(b.name));
}

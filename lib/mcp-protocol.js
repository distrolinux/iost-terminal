...
const PAPER_TRADE_TOOLS = [
  readTool('paper_trade_preflight', 'Preflight paper trade', 'Read-only paper execution preflight with a fresh server bid/ask, hard spread cap, explicit slippage tolerance, server-fill notional, cash, wallet/Pact, spend-limit and mission checks.', objectSchema({
    intentId: { type: 'string', minLength: 8, maxLength: 128, pattern: '^[A-Za-z0-9][A-Za-z0-9._:-]{7,127}$' },
    symbol: { type: 'string', minLength: 1, maxLength: 24, pattern: '^[A-Za-z0-9._/-]+$' },
    side: { type: 'string', enum: ['long', 'short'] },
  ...
  }, ['intentId', 'symbol', 'side', 'size', 'entry', 'maxSlippageBps', 'walletId', 'pactId'])),
  mutationTool('paper_trade_open', 'Open paper trade', 'Open one idempotent simulated position at the fresh bound server ask for long or bid for short. Requires an unexpired preflight fingerprint, explicit slippage tolerance, trade-paper scope, owned wallet and active wallet-bound Pact.', objectSchema({
    intentId: { type: 'string', minLength: 8, maxLength: 128, pattern: '^[A-Za-z0-9][A-Za-z0-9._:-]{7,127}$' },
    ...
    recipient: { type: ['string', 'null'], maxLength: 256 },
    protocol: { type: ['string', 'null'], maxLength: 128 },
  }, ['intentId', 'preflightFingerprint', 'symbol', 'side', 'size', 'entry', 'maxSlippageBps', 'walletId', 'pactId']), { idempotent: true }),

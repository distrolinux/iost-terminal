...
import { assessKrakenDraftFunding } from './kraken-draft-funding.js';
...
export async function verifyKrakenConnection({ apiKey, apiSecret }, { fetchFn = fetch, now = Date.now, requireReadOnly = false, includeFundingEvidence = false, includeFeeEvidence = false, draftNotionalUsd = null } = {}) {
  ...
  const call = async method => coordinateKrakenRequest(apiKey, async n => {
  ...
  try {
    ...
    if (draftNotionalUsd !== null) {
      try {
        const balance = await call('BalanceEx');
        const schedule = await call('TradeVolume');
        result.draftFunding = assessKrakenDraftFunding(balance, schedule, draftNotionalUsd);
      } catch { result.draftFunding = assessKrakenDraftFunding(null, null, draftNotionalUsd); }
    }

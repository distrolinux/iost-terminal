...
export function buildPortfolioRiskDecision({
  ...
  const opposingSymbolPosition = positions.some((position) => (
    String(position?.symbol || '').trim().toUpperCase() === symbol
    && (position?.side === 'short' ? 'short' : 'long') !== side
  ));
  ...
  const checks = [
    ...
    { code: 'same-symbol-direction-consistent', pass: !requireProtectiveStop || !opposingSymbolPosition },
  ...
  const headroom = equity > 0 ? [
    ...
    ...(requireProtectiveStop && opposingSymbolPosition ? [{ code: 'same-symbol-direction-consistent', usd: 0 }] : []),
  ] : [];
  const maximumNewOrderUsd = headroom.length ? Math.max(0, Math.min(...headroom.map((row) => row.usd))) : 0;
  const capacity = {
    available: maximumNewOrderUsd > 0 && !['drawdown-circuit-breaker', 'daily-loss-circuit-breaker', 'open-position-limit', 'protective-stop-required', 'same-symbol-direction-consistent'].includes(failure?.code),

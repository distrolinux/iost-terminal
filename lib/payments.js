...
// User picks a bundle → pays crypto to PROJECT OWNER's wallet (addresses in
...
const FILE = join(ROOT, 'data', 'payments.json');
...
function persist() {
  try {
    ...
    const tmp = `${FILE}.tmp`;
    writeFileSync(tmp, JSON.stringify({ payments }, null, 2));
    renameSync(tmp, FILE);

...
const swapUrlTargets = (value, tokenAddress) => {
  try {
    const url = new URL(String(value || ''));
    return String(url.searchParams.get('outputCurrency') || '').toLowerCase() === String(tokenAddress || '').toLowerCase();
  } catch { return false; }
};
...
export function evaluateTradingAccess(config = {}, { liveVerified = false } = {}) {
  const trading = config.trading || {};
  const checks = {
    trading_requested: trading.enabled === true,
    phase4_enabled: config.phase4Enabled === true,
    status_deployed: config.status === 'deployed',
    token_address: isSetAddress(trading.tokenAddress),
    bsc_chain: Number(trading.chainId) === 56,
    pair_address: isSetAddress(trading.pairAddress),
    factory_address: isSetAddress(trading.factoryAddress),
    quote_token: isSetAddress(trading.quoteToken),
    approved_dex: trading.dex === 'PancakeSwap',
    approved_swap_url: isApprovedSwapUrl(trading.swapUrl),
    swap_url_token_match: swapUrlTargets(trading.swapUrl, trading.tokenAddress),
    live_pair_verified: liveVerified === true,
  };
  const failed = Object.entries(checks).filter(([, pass]) => !pass).map(([name]) => name);
  return {
    ...
    tokenAddress: isSetAddress(trading.tokenAddress) ? trading.tokenAddress : '',

...
import { buildExecutionQualityDecision } from './execution-quality.js';
...
function normalizeQuote(quote, now, ttlMs, maxVenueSpreadBps) {
  ...
  const high24h = finite(quote?.high24h);
  const low24h = finite(quote?.low24h);
  const health = quote?.venueHealth || {};
  ...
  return {
    ...
    sampleCount: Number.isSafeInteger(Number(health.sampleCount)) ? Number(health.sampleCount) : 0,
    reliabilityPct: finite(health.reliabilityPct) ?? 100,
    consecutiveFailures: Number.isSafeInteger(Number(health.consecutiveFailures))
      ? Number(health.consecutiveFailures) : 0,
    circuitOpen: health.circuitOpen === true,
    high24h: high24h > 0 && low24h > 0 && high24h >= low24h ? high24h : null,
    low24h: high24h > 0 && low24h > 0 && high24h >= low24h ? low24h : null,
  };
}
...
export function buildMultiVenueExecutionQuote({
  ...
  const executionQuality = buildExecutionQualityDecision({ side, venues: trusted });
  const route = trusted.find((quote) => quote.source === executionQuality.selectedVenue) || null;
  ...
  const quoteIntegrity = {
    ...
    venues: trusted.map((quote) => ({
      ...
      high24h: round(quote.high24h, 8),
      low24h: round(quote.low24h, 8),
    })),
  };
  if (!quorumMet || !route) {
    return {
      ok: false,
      reasonCode: quorumMet ? executionQuality.reasonCode : 'quote-quorum',

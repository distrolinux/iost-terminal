...
//
// PROJECT OWNER-approved model (2026-08-10): every customer can create API keys for
...
export function resolve(apiKey) {
  ...
  const hit = Object.values(store.byId).find((k) => k.hash === h);
...
export function verifySecret(id, secret) {
  ...
  if (k.hash !== sha256(secret)) return null;

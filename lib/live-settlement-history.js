  ...
  return { append(ownerId, binding, evidence) {
    ...
    try {
      ...
      for (const link of next.links) {
        ...
        if (old) { if (old !== encoded) return held; continue; }
        if (link.ledgerIds.some(key => ledgers.has(key))) return held;
        additions.push(link);
      }
      if (!additions.length) return { ...held, status: 'replay' };
      ...
      sync(directory); return { ...held, status: 'recorded', newEvidenceFills: additions.length };
    } catch { return held; }
  } };
}

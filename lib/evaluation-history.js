...
function summary(run) {
  ...
  return {
    ...
    promotion: evaluation.promotion, benchmark: evaluation.benchmark || null, challenge: evaluation.challenge || null, evidence: evaluation.evidence,
  };
}
...
export function listAgentBenchmarks(ownerId, limit = DEFAULT_MAX_RUNS, now = Date.now()) {
  const history = listEvaluations(ownerId, limit, now);
  return {
    mode: 'paper-only',
    generatedAt: Math.trunc(now),
    benchmarks: history.runs.map((run) => ({
      runId: run.id,
      createdAt: run.createdAt,
      symbol: run.symbol,
      timeframe: run.timeframe,
...
export function listAgentChallenges(ownerId, limit = DEFAULT_MAX_RUNS, now = Date.now()) {
  const history = listEvaluations(ownerId, limit, now);
  return {
    mode: 'paper-only', generatedAt: Math.trunc(now),
    challenges: history.runs.map((run) => ({ runId: run.id, createdAt: run.createdAt, symbol: run.symbol,
      timeframe: run.timeframe, strategy: run.strategy, challenge: run.challenge || null })),
    retention: history.retention,
    guarantees: { readOnly: true, deterministic: true, privateByDefault: true, ownerIsolated: true,
      automaticPromotion: false, executionAuthority: 'none', liveScopeUsed: false, publicChainUsed: false },
  };
}

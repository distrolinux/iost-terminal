...
import { buildSignedTx, execTx, sendSignedTx, pinConfig, publicChainActionsAvailable } from './chain.js';
...
export async function requestCreation({ userId, accountName, publicKey }) {
  if (!publicChainActionsAvailable()) {
    return { ok: false, status: 403, error: 'public-chain wallet creation is unavailable in the paper-only launch' };
  }
...
export async function createAccountOnChain(userId) {
  if (!publicChainActionsAvailable()) return { ok: false, error: 'public-chain actions are unavailable in the paper-only launch' };
  const cfg = walletConfig();

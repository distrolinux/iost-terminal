// lib/iost-accounts.js — subsidized IOST mainnet wallets (free account per user)
//
// PROJECT OWNER-approved model (2026-08-10): every registered user gets a REAL IOST
// mainnet account; the PLATFORM pays the creation fee (~11 IOST: 10 IOST gas
// pledge + ~1 IOST tx gas). The landing page already advertises this — the
// hero badge says "Every account comes with a real IOST wallet — we pay the
// fee" (do not touch the landing).
//
...
//   gas:      contract auto-pledges 10 IOST from the referrer to the new
//             account's gas allowance (gas.iost/pledge via callWithAuth) —
//             that is the bulk of the ~11 IOST fee
...
export const FEE_IOST = 11; // honest label: 10 IOST gas pledge + ~1 IOST tx gas

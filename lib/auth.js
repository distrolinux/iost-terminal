...
import { readFileSync, writeFileSync, renameSync, mkdirSync, existsSync, chmodSync, openSync, closeSync, fsyncSync, unlinkSync } from 'node:fs';
...
// Tests and one-off audits can point auth at a scratch store. Production keeps
// using /app/data through its existing bind mount when this is unset.
...
const MAX_PASSWORD_BYTES = 72; // bcrypt ignores bytes beyond this boundary

function passwordError(password) {
  if (typeof password !== 'string' || password.length < MIN_PASSWORD)
    return `password must be at least ${MIN_PASSWORD} characters`;
  if (Buffer.byteLength(password, 'utf8') > MAX_PASSWORD_BYTES)
    return `password must be at most ${MAX_PASSWORD_BYTES} UTF-8 bytes`;
  return null;
}
...
/** Credential maintenance runs synchronously through this sole store writer.
 * Prepare and verify the entire replacement on disk before atomic commit.
 * The backup is written by the caller before entering this function.
 */
export function persistCredentialReplacement(user, replacement, status) {
  if (!user || findById(user.id) !== user) throw Error('account changed');
  mkdirSync(DATA_DIR, { recursive: true });
  const tmp = `${FILE}.credential-${crypto.randomUUID()}.tmp`;
  const updated = { ...user, krakenKey: replacement };
  if (status !== undefined) updated.krakenKeyStatus = status;
  if (replacement === null) { delete updated.krakenKey; delete updated.krakenKeyStatus; }
  const contents = JSON.stringify(users.map(u => u === user ? updated : u), null, 2);
  let fd;
  try {
    fd = openSync(tmp, 'wx', 0o600);
    writeFileSync(fd, contents);
    fsyncSync(fd);
    if (readFileSync(tmp, 'utf8') !== contents) throw Error('write verification failed');
    closeSync(fd); fd = undefined;
    renameSync(tmp, FILE);
    user.krakenKey = replacement;
    if (status !== undefined) user.krakenKeyStatus = status;
    if (replacement === null) { delete user.krakenKey; delete user.krakenKeyStatus; }
    // Keep memory aligned if a post-rename durability sync fails. Caller reports
    // failure and requires a fresh inspection rather than replaying the plan.
    const directory = openSync(DATA_DIR, 'r');
...
export async function registerUser(emailRaw, password) {
  ...
  if (invalidPassword) return { ok: false, status: 400, error: invalidPassword };

...
//   signup           +1   per registered user; provisional until an explicit
//                         account-activation transition
...
export const EVENTS = {
  signup: { points: 1, label: 'User signup' },
...
export function getEligibleBalance(ownerId) {
  if (!ownerId) return 0;
  return store.ledger
    .filter((e) => e.ownerId === ownerId && e.meta?.provisional !== true)
    .reduce((sum, e) => sum + (e.points || 0), 0);
}

export function getPointBalances(ownerId) {
  const total = getBalance(ownerId);
  const eligible = getEligibleBalance(ownerId);
  return { total, eligible, provisional: total - eligible };
}

/** Deterministic positive eligible balances as they stood at a UTC millisecond cutoff. */
export function getEligibleBalancesAt(cutoff) {
  const cutoffMs = Number(cutoff);
  if (!Number.isSafeInteger(cutoffMs) || cutoffMs < 0) return [];
  const balances = new Map();
  for (const entry of store.ledger) {
    if (!entry.ownerId || !Number.isFinite(entry.ts) || entry.ts > cutoffMs) continue;
    if (entry.meta?.provisional === true) continue;
    if (Number.isFinite(entry.meta?.activatedAt) && entry.meta.activatedAt > cutoffMs) continue;
    balances.set(entry.ownerId, (balances.get(entry.ownerId) || 0) + (entry.points || 0));
  }
  return [...balances.entries()]
    .filter(([, points]) => Number.isSafeInteger(points) && points > 0)
    .sort(([ownerA], [ownerB]) => ownerA < ownerB ? -1 : ownerA > ownerB ? 1 : 0)
    .map(([ownerId, points]) => ({ ownerId, points }));
}
...
export function debitForConversion({ ownerId, points, claimId, txHash }) {
  ...
  if (getEligibleBalance(ownerId) < amount) return { ok: false, error: 'insufficient eligible points' };
  const entry = {
...
// ---------------------------------------------------------------------------
// signup award + activation
// ---------------------------------------------------------------------------
/** Award one provisional signup point per owner. Safe to retry. */
export function awardSignup(ownerId) {
  if (!ownerId) return { ok: false, error: 'ownerId required' };
  const existing = store.ledger.find((e) => e.ownerId === ownerId && e.event === 'signup');
  if (existing) return { ok: true, entry: existing, already: true };
  return credit({ ownerId, event: 'signup', refId: ownerId, meta: { provisional: true } });
}

/**
 * Mark an existing signup award eligible after the caller has established the
 * account's approved activation criteria. This helper does not define identity
 * or KYC policy and is safe to retry.
 */
export function activateSignup(ownerId) {
  if (!ownerId) return { ok: false, error: 'ownerId required' };
  const entry = store.ledger.find((e) => e.ownerId === ownerId && e.event === 'signup');
  if (!entry) return { ok: false, error: 'signup award not found' };
  if (entry.meta?.provisional !== true) return { ok: true, entry, already: true };
  entry.meta = { ...(entry.meta || {}), provisional: false, activatedAt: Date.now() };
  save();
  return { ok: true, entry };
}

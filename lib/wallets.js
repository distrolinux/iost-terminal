...
// Lifetime grant for the self-service Agent Launchpad. These are internal
// simulation credits only: no cash value, token balance, withdrawal path, or
// public-chain representation exists. Persisting the lifetime total on the
// parent wallet prevents repeated setup calls from minting unlimited credits.
export const PAPER_ONBOARDING_CREDIT_CAP_MINOR = 10_000; // $100.00
...
export function createAgentWallet({ ownerId, name, limits, capabilities = [], regions = [], approvalRequired = false, origin = null }) {
  if (zeroAddr(ownerId)) throw new Error('ownerId required');
  ...
  const w = {
    ...
    origin: origin === 'self-service-launchpad' ? origin : null,
    status: 'active',
    balances: { USD: 0 },
    createdAt: Date.now(),
  };
...
/** Grant a bounded lifetime allotment of paper-only onboarding credits. */
export function grantPaperOnboardingCredits(ownerId, amountMinor) {
  const w = ensureUserWallet(ownerId);
  const amount = Math.trunc(Number(amountMinor));
  if (!Number.isSafeInteger(amount) || amount <= 0) throw new Error('paper onboarding credit amount must be a positive integer');
  const already = Math.max(0, Math.trunc(Number(w.paperOnboardingGrantedMinor) || 0));
  const remaining = Math.max(0, PAPER_ONBOARDING_CREDIT_CAP_MINOR - already);
  if (amount > remaining) {
    throw new Error(remaining > 0
      ? `paper onboarding credit request exceeds remaining lifetime limit (${remaining} minor)`
      : 'paper onboarding credit limit reached');
  }
  w.paperOnboardingGrantedMinor = already + amount;
  w.balances.USD = (w.balances?.USD || 0) + amount;
  save();
  return {
    ok: true,
    walletId: w.walletId,
    grantedMinor: amount,
    lifetimeGrantedMinor: w.paperOnboardingGrantedMinor,
    remainingGrantMinor: PAPER_ONBOARDING_CREDIT_CAP_MINOR - w.paperOnboardingGrantedMinor,
    balanceMinor: w.balances.USD,
  };
}

// lib/onchain.js — IOST blockchain on-chain dashboard via public RPC (api.iost.io)
...
async function rpc(path) {
  ...
  try {
    const res = await fetch(`${RPC}${path}`, { signal: ctl.signal });
...
export async function getChainSnapshot(force = false) {
  ...
  let nodeInfo = null, chainInfo = null;
  try { nodeInfo = await rpc('/getNodeInfo'); } catch { /* offline */ }
  try { chainInfo = await rpc('/getChainInfo'); } catch { /* offline */ }

  const headBlock = chainInfo?.head_block != null ? +chainInfo.head_block : 0;
  const latencyMs = chainInfo?.head_block_time && chainInfo?.lib_block_time
    ? (+chainInfo.head_block_time - +chainInfo.lib_block_time) / 1e6 : null;
  ...
  const snapshot = {
    chain: {
      netName, headBlock, latencyMs: latencyMs != null ? Math.round(latencyMs * 100) / 100 : null,
      peerCount,
      tps: Math.round(tps * 100) / 100,
      avgTxPerBlock: Math.round(avgTxPerBlock * 100) / 100,
      activeAddresses: addresses.size,
      transactions24h: txCount,

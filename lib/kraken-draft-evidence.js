...
export async function krakenSystemEvidence({ fetchFn = (...args) => fetch(...args), now = Date.now } = {}) {
  const started = now();
  try {
    const data = await publicJson(fetchFn, 'SystemStatus', null, AbortSignal.timeout(8000));
    const observedAt = now(), sourceAt = Date.parse(data.timestamp);
    if (!['online', 'maintenance', 'cancel_only', 'post_only'].includes(data.status) || !Number.isFinite(sourceAt) || sourceAt > observedAt + 5000 || observedAt - sourceAt > 30000 || observedAt < started || observedAt - started > 8000) throw Error('unavailable');
    for (const key of ['emergency', 'upcoming_maintenance']) if (data[key] !== undefined && !Array.isArray(data[key])) throw Error('invalid advisory');
    return { status: data.status, emergencyCount: data.emergency?.length || 0, maintenanceCount: data.upcoming_maintenance?.length || 0, observedAt, expiresAt: Math.min(started + 30000, sourceAt + 30000), executionAuthority: 'none' };
  } catch { return { status: 'unavailable', executionAuthority: 'none' }; }
}

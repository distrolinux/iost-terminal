...
import { IOST_RPC, deriveIostNodeHealth, iostChainIdentity, iostNodePublicConfig } from './iost-node.js';
...
export async function chainStatus() {
  ...
  try {
    ...
    return {
      ...
      identity: iostChainIdentity(),
      node: { ...iostNodePublicConfig(), ...deriveIostNodeHealth(info, nodeInfo, Date.now(), Date.now() - started) },
    };
  } catch (e) {
    return {
      ok: false, chain: CHAIN_LABEL, error: e.message, configured: pinConfigured(), identity: iostChainIdentity(),

...
const RPC = process.env.IOST_RPC || 'https://api.iost.io';
...
async function rpcGet(path, timeoutMs = 15000) {
  ...
  try {
    const res = await fetch(`${RPC}${path}`, { signal: ctrl.signal, headers: { accept: 'application/json' } });
...
export async function chainStatus() {
  try {
    const info = await rpcGet('/getChainInfo');
    return {
      ok: true, chain: CHAIN_LABEL, chainId: info.chain_id,
      headBlock: info.head_block, net: info.net_name,
      configured: pinConfigured(),
      rpc: RPC,
    };
  } catch (e) {
    return { ok: false, chain: CHAIN_LABEL, error: e.message, configured: pinConfigured(), rpc: RPC };
  }
}

...
export function credentialStorageStatus(user) {
  ...
  let activeKeyId;
  try { activeKeyId = configuredCredentialVault().activeKeyId; vaultConfigured = true; } catch { /* unavailable */ }
  ...
  const migrationNeeded = Boolean(blob) && (format !== 'account-bound-v1' || blob.keyId !== activeKeyId);
...
export function createCredentialMaintenance({ backup, persist, now = Date.now } = {}) {
  ...
  return {
    ...
    apply(user, sessionId, token, confirmed) {
      ...
      plans.delete(token);
      let activeKeyId;
      try { activeKeyId = configuredCredentialVault().activeKeyId; } catch { return fail('credential-or-vault-changed'); }
      if (JSON.stringify(user.krakenKey) !== plan.before || plan.replacement.keyId !== activeKeyId) return fail('credential-or-vault-changed');
      const original = getUserKrakenKeys(user);

...
export function authRouter(siteUrl = 'https://iostcallister.com') {
  ...
  // Keep bearer-like reset tokens out of URLs, browser history and proxy logs.
  r.post('/reset/validate', credsLimiter, (req, res) => {
    const found = auth.validateResetToken(req.body?.token);

...
export function authRouter() {
  ...
  r.post('/register', async (req, res) => {
    ...
    res.status(201).json({ user: auth.toPublic(result.user), session: logged, refAward });
  });
  ...
  r.post('/login', credsLimiter, async (req, res) => {
    ...
    if (result.ok) {
      await loginSession(req, result.user);
      return res.json({ user: auth.toPublic(result.user) });
    }
    if (result.totpRequired) return res.json({ totpRequired: true, email: result.user.email });
  ...
  r.post('/login/totp', credsLimiter, async (req, res) => {
    ...
    if (!user || !user.totpEnabled) return res.status(401).json({ error: 'invalid email or code' });
    const check = auth.checkTotp(user, req.body?.totpCode);
    if (!check.ok) return res.status(401).json({ error: 'invalid email or code' });
    await loginSession(req, user);
    res.json({ user: auth.toPublic(user), usedBackup: !!check.usedBackup });
  });
  ...
  r.get('/me', (req, res) => {
    ...
    if (req.agentKey) return res.json({ agent: true, key: req.agentKey });
  ...
  r.post('/reset/request', async (req, res) => {
    ...
    if (user) {
      ...
      await auth.sendResetEmail(user.email, token, req.get('host'));
    }

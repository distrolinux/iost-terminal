...
export function verifyLiveFillEvidence(order, trades) {
  ...
  try {
    ...
    return { ...unknown, status: ids.length ? 'fill-totals-matched' : 'no-fills-reported', fillCount: ids.length, reportedFilledQuantity: order.filledQuantity, reportedCost: order.cost, reportedFee: order.fee, feeSettlementVerified: false };

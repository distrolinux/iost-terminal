...
export function buildAgentExecutionReadiness({
  ...
  const checks = agentRequired ? [
    ...
    { code: 'execution-reconciliation-clear', pass: reconciliation?.decision === 'allow' },
  ...
  return {
    ...
    policy: {
      ...
      executionReconciliationRequired: agentRequired,
      failClosed: true,
    ...
    evidence: {
      ...
      reconciliation: {
        status: reconciliation?.status || 'unavailable',
        decision: reconciliation?.decision || 'deny',
        reasonCode: reconciliation?.reasonCode || 'execution-reconciliation-unavailable',
        criticalFindings: Number(reconciliation?.counts?.criticalFindings || 0),
        warningFindings: Number(reconciliation?.counts?.warningFindings || 0),
        receiptChainVerified: reconciliation?.evidence?.receiptChainVerified === true,
        cashInvariant: reconciliation?.evidence?.cashInvariant === true,
      },

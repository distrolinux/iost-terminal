...
const allowedSloStatus = (status) => ['healthy', 'warming-up'].includes(status);
...
export function buildAgentExecutionReadiness({
  ...
  const checks = agentRequired ? [
    ...
    { code: 'safety-budget-available', pass: allowedSloStatus(safetySlo?.status) && safetySlo?.errorBudget?.exhausted !== true },
  ...
  return {
    ...
    policy: {
      ...
      allowedSloStatuses: ['warming-up', 'healthy'],

...
import { mountOrderReview } from '/js/order-review.js?v=7';
...
async function renderExchangeConnections() {
  ...
  try {
    ...
    el.innerHTML = `
      ...
      <section class="card" aria-labelledby="settlementReviewTitle"><h2 id="settlementReviewTitle">Settlement evidence review · Read-only</h2>
      <p>${esc(({ 'not-observed': 'No local settlement evidence recorded. This does not mean an order settled.', 'evidence-recorded': 'Local evidence recorded. Final settlement is not verified.', 'review-required': 'Evidence conflict requires owner review. The hold remains in place.' })[s.settlementReview?.status] || 'Settlement evidence is unavailable. Do not assume it is clear.')}</p>
      <dl><dt>History integrity</dt><dd>${s.settlementReview?.historyIntegrity === 'verified-local-chain' ? 'Local evidence chain checked; not exchange settlement confirmation' : 'Not verified'}</dd>
      <dt>Review reason</dt><dd>${esc(({ 'recorded-fill-changed': 'Previously recorded fill details changed', 'recorded-ledger-reused': 'A ledger entry was reused', 'settlement-review-marker-unavailable': 'Review marker could not be read', 'no-recorded-settlement-conflict': 'No recorded conflict in this snapshot', 'no-local-settlement-evidence': 'No local evidence' })[s.settlementReview?.reasonCode] || 'Evidence requires inspection')}</dd>
      <dt>Recorded fills / ledger entries</dt><dd>${Number.isSafeInteger(s.settlementReview?.fillCount) ? s.settlementReview.fillCount : 'Unknown'} / ${Number.isSafeInteger(s.settlementReview?.ledgerCount) ? s.settlementReview.ledgerCount : 'Unknown'}</dd></dl>
      <p class="muted">This reads private local records only. It does not contact Kraken, collect new evidence, clear holds or authorize trading. Use Refresh evidence above to reread the summary. If a conflict is shown, preserve the records for operator review; do not delete the hold.</p></section>
      ...
      ${c.configured ? '<label><input type="checkbox" id="includeKrakenFeeEvidence"> Include account-specific BTC/USD fee schedule</label><p>Optional read-only TradeVolume query. Only maker/taker percentages are shown; trading volume is not retained. IOST platform fee: $0. This does not verify order affordability or the final fee.</p>' : ''}
    ...
    $('#verifyKrakenConnection')?.addEventListener('click', async (event) => {
      ...
      try {
        const result = await post('/api/exchange-connections/kraken/verify', { includeFundingEvidence: $('#includeKrakenFundingEvidence')?.checked === true, includeFeeEvidence: $('#includeKrakenFeeEvidence')?.checked === true });
        if (generation !== connectionRequestGeneration) return;
        ...
        if (result.feeEvidence) {
          const fee = result.feeEvidence;
          output.textContent += fee.status === 'schedule-observed' ? ` BTC/USD account schedule: maker ${fee.makerPercent}%, taker ${fee.takerPercent}%.` : ' BTC/USD account fee schedule: unavailable; not zero.';
          output.textContent += ' IOST platform fee: $0. Snapshot only, not a final fee or approval. Do not apply to other pairs. A limit order can incur taker fees. Eligibility and affordability remain unverified.';
        }
      } catch {
      ...
      ${receipts.length ? `<div class="table-wrap"><table>
        ...
        <tbody>${receipts.map(r => {
          ...
          return `<tr>
            ...
            <td>${esc(authority)}<small>${esc(riskEvidence)} · paper-only · live unused</small></td>
